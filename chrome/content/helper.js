var PsicotsiHelper = { MODULE_NAME : "Helper", PAGES : new Array('myhattrick'), DEFAULT_ENABLED : true, init : function() { }, run : function( page, doc ) { this.getOwnTeamInfo(doc); }, change : function( page, doc ) { }, findTeamId : function(element) { var links = element.getElementsByTagName('a'); for (var i=0; i < links.length; i++) { if ( links[i].href.match(/TeamID=/i) ) { return links[i].href.replace(/.+TeamID=/i, "").match(/^\d+/)[0]; } } return false; }, findSecondTeamId : function(element,firstteamid) { var links = element.getElementsByTagName('a'); for (var i=0; i < links.length; i++) { if ( links[i].href.match(/TeamID=/i) ) { var id=links[i].href.replace(/.+TeamID=/i, "").match(/^\d+/)[0]; if (id!=firstteamid) return id; } } return 0; }, findPlayerId : function(element) { var links = element.getElementsByTagName('a'); for (var i=0; i < links.length; i++) { if ( links[i].href.match(/playerID=/i) ) { return links[i].href.replace(/.+playerID=/i, "").match(/^\d+/)[0]; } } return null;
}, findYouthPlayerId : function(element) { var links = element.getElementsByTagName('a'); for (var i=0; i < links.length; i++) { if ( links[i].href.match(/YouthPlayerID=/i) ) { return links[i].href.replace(/.+YouthPlayerID=/i, "").match(/^\d+/)[0]; } } return null; }, getSkillLevelFromLink : function(link) { var value = link.href.replace(/.+(ll|labellevel)=/i, "").match(/^\d+/); return value; },
...
getValueFromLink : function (link) {
  ...
  } catch (e) { throw("In function getValueFromLink()\n - " + e); }
  return skill*1;
},  
...
getInfoTable : function (doc) {
  ...
  } catch (e) { throw("In function getInfoTable()\n - " + e); }
  return infoTable;
},
...
getTableExist : function (doc) {
  ...
  try {
  	...
  	if (numberLinks > 13){table=true;}
  	else {table=false;}
  	
  } catch (e) { table = false;throw("In function getTableExist()\n - " + e); }
  return table;
...
getTSI : function (infoTable) {
  ...
  } catch (e) { throw("In function getTSI()\n - " + e); }
  return tsi*1;
},
...
getWage : function (infoTable) {
  ...
  try {
   ...
   var rate = 1.0;
    ...
    if (infoTable.rows[2].cells[1].textContent.match(/\%/i)){wage = wage/1.2;} wage += 250;
      
      
   

  } catch (e) { throw("In function getWage()\n - " + e); }
...
getInjuries : function (infoTable) {
  try {
    ...
    if (container.textContent.search(/\d+/) > -1)
      return true;
  } catch (e) { throw("In function getInjuries()\n - " + e); }
  return false;
},
...
getAge : function (doc) {
  ...
  } catch (e) { dump("In function getAge()\n - " + e); }
  return age*1;
},
...
simWage : function (skillValue, mainSkill, sta) {
	var salary = 0;
	var WageArray = new Array(new Array(2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004),
							  new Array(2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004),
							  new Array(2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004),
							  new Array(2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004),
							  new Array(2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004,2.698970004),
							  new Array(2.716003344,2.716003344,2.707486674,2.707486674,2.707486674,2.728492395,2.716003344,2.707486674,2.716003344),
							  new Array(2.748188027,2.732393760,2.732393760,2.732393760,2.732393760,2.748657963,2.732393760,2.732393760,2.732393760),
							  new Array(2.819543936,2.806179974,2.806179974,2.778151250,2.778151250,2.789658988,2.792391689,2.763427994,2.792391689),
							  new Array(2.954242509,2.934498451,2.911433147,2.874361836,2.846766673,2.911193307,2.934498451,2.872617546,2.903089987),
							  new Array(3.170000000,3.139879086,3.116813782,3.079742471,3.052147309,3.104423127,3.139879086,3.041392685,3.107209970),
							  new Array(3.397940009,3.365487985,3.342422681,3.305351369,3.277756207,3.266439474,3.354108439,3.179542656,3.313867220),
							  new Array(3.643452676,3.617418939,3.591497524,3.556302501,3.528707338,3.454303288,3.595496222,3.317692628,3.546542663),
							  new Array(3.893428615,3.863700000,3.833784375,3.788805636,3.761210473,3.674115352,3.833784375,3.455842599,3.788168371),
							  new Array(4.120573931,4.091595803,4.070000000,4.025021261,3.997426099,3.893923757,4.062205809,3.593992570,4.011993115),
							  new Array(4.343605508,4.310000000,4.283865660,4.238886921,4.211291759,4.095261523,4.280760369,3.732142541,4.231979027),
							  new Array(4.552327416,4.517313266,4.503518313,4.458539574,4.430944412,4.307902055,4.486288206,3.870292513,4.436798510),
							  new Array(4.747970819,4.694254112,4.680459159,4.635480420,4.607885258,4.492329889,4.679427897,4.008442484,4.621757675),
							  new Array(4.926959488,4.904239142,4.870000000,4.825021261,4.797426099,4.662981108,4.856002845,4.146592455,4.810501348),
							  new Array(5.095977842,5.056599976,5.027066049,4.982087311,4.954492148,4.815855091,5.040850938,4.284742426,4.971832280),
							  new Array(5.252916130,5.213538264,5.184004337,5.139025598,5.111430436,4.971117953,5.267171728,4.422892397,5.138807765),
							  new Array(5.417836912,5.350000000,5.320466073,5.275487334,5.247892172,5.291071343,5.345116665,4.561042369,5.355643050),
							  new Array(5.417836912,5.350000000,5.320466073,5.275487334,5.247892172,5.291071343,5.345116665,4.561042369,5.355643050));
...
calcMaxSkillWage : function (playerskills, salary) {
	...
	// Recognising mainSkill
	switch (pskillMax) {
		case 2: mainSkill = "PM"; break;
		case 3: mainSkill = "WG"; break;
		case 4: mainSkill = "SC"; break;
		case 6: mainSkill = "PS"; break;
		case 7: mainSkill = "DF"; break;
	}
	// Starting approximation
...
if (subForm == "Low"){var odc = 0.01255;}
if (subForm == "Avg"){var odc = 0;}
if (subForm == "High"){var odc = -0.01255;}


var form = (FORM * 0.025) + 0.100;
try {
var skill = ((Math.pow((100*TSI)/(form), 1/3.4))/10+1);
}catch(e){Psicotsi.alert('ERROR #159347> \n'+e);}
