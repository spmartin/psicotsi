...
Psicotsi.core_modules = [ PsicotsiPrefs,
                          Psicotsil10n ];
Psicotsi.news = [];


Psicotsi.globals=[];
for (Psicotsi.global in this){Psicotsi.globals.push(Psicotsi.global);} 
...
var PsicotsiMain = {
    ...
    init : function() { 
		...
		for (var  i in Psicotsi.modules ) {
			...
			if ( module.MODULE_NAME
                   && Psicotsi.isModuleEnabled( module ) )
			{
				if (module.init)
				{
					try {
    ...
    registerOnPageLoad : function(document) {
		...
		var statusbarDeactivate = document.getElementById(
			"psicotsi_statusbar_deactivate" );
		statusbarDeactivate.setAttribute( "label", Psicotsil10n.getString(
			"psicotsi.prefs.disableTemporaryLabel") );
		...
		toolsMenu.setAttribute( "label", Psicotsil10n.getString( 
			"psicotsi.menu.configurepsicotsi") );
	...
	try {  	
		...
		if( (!( PsicotsiPrefs.getBool("disableOnStage") &&
			Psicotsi.getHref( doc).search( stage_regexp ) > -1))
			&& ( !PsicotsiPrefs.getBool("disableTemporary")) ) {
        ...
        Psicotsi.reload_module_css(doc);
			...
			for ( var i in Psicotsi.ht_pages ) {
				if ( Psicotsi.isPage( Psicotsi.ht_pages[i], doc ) ) {
					...
					Psicotsi.may_run_on_page[i].forEach(
						function( fn ) {
								Psicotsi.run_on_cur_page.push({'page':i,'module':fn});								
						} );							
				}
			}
			...
			var stage_regexp = /http:\/\/stage\.hattrick\.org/i;
			if( PsicotsiMain.new_start && ((( PsicotsiPrefs.getBool("disableOnStage") &&
				Psicotsi.getHref( doc).search( stage_regexp ) != -1))
				|| ( PsicotsiPrefs.getBool("disableTemporary"))) ) { 			
...
try {
    ...
    if ( module.run ) {
		for (var i=0;i<module.PAGES.length;++i) {
			...
			//dump(module.PAGES[i]+'\n');
			if (Psicotsi.isModuleEnabled( module ) )
				Psicotsi.run_on_page[module.PAGES[i]].push( module );
		}
    }
...
Psicotsi.reload_module_css = function(doc) {  	dump('reload permanents css\n');
			// check permanant css
						...
						for (var k=0; k<module.OPTIONS_CSS.length;++k ) {
							...
							{	if (module.OPTIONS_CSS[k] != "" && (!isRTL || !module.OPTIONS_CSS_RTL)) {
							 		if (module.OPTIONS_CSS_RTL && module.OPTIONS_CSS_RTL[k] != "") 
											Psicotsi.unload_css_permanent ( module.OPTIONS_CSS_RTL[k]) ; 	
									Psicotsi.load_css_permanent ( module.OPTIONS_CSS[k] ) ;  
								}
								else {
									if (module.OPTIONS_CSS[k] != "") 
											Psicotsi.unload_css_permanent ( module.OPTIONS_CSS[k] ) ; 	
									if (isRTL) {
										if (module.OPTIONS_CSS_RTL && module.OPTIONS_CSS_RTL[k] != "") 
											Psicotsi.load_css_permanent ( module.OPTIONS_CSS_RTL[k] ) ; 
									}
									else {
										if (module.OPTIONS_CSS_RTL && module.OPTIONS_CSS_RTL[k] != "") 
											Psicotsi.unload_css_permanent ( module.OPTIONS_CSS_RTL[k] ) ; 
...
Psicotsi.addBoxToSidebar = function( doc, newBoxHeader, newBoxContent, boxId,
	...
	if( Psicotsi.hasElement( doc, boxId ) ||
		Psicotsi.hasElement( doc, boxContentId )) {
		return;
	}
...
Psicotsi.getStatusBar = function(ev) {
