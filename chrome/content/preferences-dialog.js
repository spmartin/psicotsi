...
setStatusBar : function() {
		...
		var modules_list = document.getElementById( 'status-bar' );
		//modules_list.setAttribute( "style", "background-color:ButtonFace !important; color: ButtonText !important;");

	if (PsicotsiPrefs.getBool( "statusbarshow" )) {

		var statusBP = document.createElement('statusbarpanel');
		statusBP.setAttribute('id','psicotsi-status-bar-image');
    statusBP.setAttribute('context','psicotsi-status-bar-menu');
    
    var image = document.createElement('image');
    image.setAttribute('id','psicotsi-status-bar-img');
    image.setAttribute('style','idth:16px; height: 16px; cursor: pointer; list-style-image: url(chrome://psicotsi/skin/psicotsi.gif);');
    image.setAttribute('tooltiptext','Psicotsi');
    image.setAttribute('onclick','PsicotsiPreferencesDialog.configurePsicotsi(event.button);');
    
    
    statusBP.appendChild(image);
    
    var menup = document.createElement('menupopup');
    menup.setAttribute('id','psicotsi-status-bar-menu');
    
    var menui1 = document.createElement('menuitem');
    menui1.setAttribute('id','psicotsi_statusbar_config_menu');
    menui1.setAttribute('oncommand','PsicotsiPreferencesDialog.configurePsicotsi();');
    
    var menui2 = document.createElement('menuitem');
    menui2.setAttribute('id','psicotsi_statusbar_reload');
    menui2.setAttribute('oncommand','PsicotsiService.load();');    
    
    var menui3 = document.createElement('menuitem');
    menui3.setAttribute('id','psicotsi_statusbar_deactivate');
    menui3.setAttribute('oncommand','PsicotsiPreferencesDialog.deactivate();'); 
    menui3.setAttribute('type','checkbox');
    menui3.setAttribute('autocheck','true');
      
      menup.appendChild(menui1); 
      menup.appendChild(menui2);
      menup.appendChild(menui3);
     statusBP.appendChild(menup);
     modules_list.appendChild(statusBP);
   }
	...
	initCaptionsAndLabels : function( document ) {
		...
		// Captions and labels
		var allLabels = [ "MainTab", "AboutTab",
						  "buttonSave", "buttonCancel" ];
		for(var i = 0; i < allLabels.length; i++) {
			var thisElement = document.getElementById(allLabels[i]);
			thisElement.setAttribute( "label", Psicotsil10n.getString(
				"psicotsi.prefs." + allLabels[i]) );
		}
	},
  ...
  return boolPrefs;
    ...
    } catch (e){Psicotsi.alert(' error prefDlg.js > \n' + e);}   
    ...
    try{
    document.getElementById("psico-version-num").value = "PsicoTSI Terminus Edition v."+ version;
    ...
    document.getElementById('Label_As_Is').value = Psicotsil10n.getString("psicotsi.prefs.distribAsIs");
		...
		modules_list.setAttribute("style", "background-color:ButtonFace !important; color: ButtonText !important;");
    var vbox = doc.createElement("vbox");
    }catch(e){Psicotsi.alert(' error #51147 >\n' + e);}
try{
//developers
        var spacer = document.createElement( "spacer" );
        spacer.setAttribute('flex','1');
...
}catch (e){Psicotsi.alert('error initAboutPref() >\n' + e);}
	},
         ...
         for each ( cat in Psicotsi.moduleCategories ) {
            ...
            default : continue;
				...
				for ( var i = 0; i < modules_list.childNodes.length; ++i ) {
					...
					PsicotsiPreferencesDialog.setModuleEnableState( modules_list.childNodes[i].prefname,
					...
					if (modules_list.childNodes[i].radio) {
						...
						for (var j = 0; j < checkboxes.length; j++) {
							if (checkboxes[j].id.search(/_text$/) == -1)
								PsicotsiPreferencesDialog.setModuleEnableState( modules_list.childNodes[i].prefname + "." + checkboxes[j].id, checkboxes[j].checked );
							else
								PsicotsiPreferencesDialog.setModuleOptionsText( modules_list.childNodes[i].prefname + "." + checkboxes[j].firstChild.id, checkboxes[j].firstChild.value );
						}
					}
						}
        }
...
PsicotsiPreferencesDialog.configurePsicotsi = function( button ) {
	if(!button) {
        window.open("chrome://psicotsi/content/preferences-dialog.xul",
...
PsicotsiPreferencesDialog.prefhelp_show = function ( HelpTitle, HelpDesc, where ) {
    openDialog("chrome://psicotsi/content/preferences-help.xul",
               "PsicoTSI Help",
...
PsicotsiPreferencesDialog.SavePrefs = function (ev) {
        try {
			...
			for(var i = 0; i < array.length; i++) {					
				if ((PsicotsiPreferencesDialog.isPrefSetting(array[i]) && document.getElementById("saveprefsid").checked)
					|| (!PsicotsiPreferencesDialog.isPrefSetting(array[i]) && document.getElementById("savenotesid").checked)) {
					...
					else { value=PsicotsiPrefs.getInt(array[i]);
